...
export class Web3ModalConnector {
    ...
    constructor({ useChipnet = false, globalClient, web3Modal, relayerRegion, logger, }) {
    ...
    async reset() {
        ...
        this.accounts = [];
        this.chains = [];
        this.client?.removeAllListeners(undefined);
    ...
    ;
    async onSessionConnected(_session) {
    ...
    async _subscribeToEvents(_client) {
    ...
    ;
    async _checkPersistedState(_client) {
        if (typeof _client === "undefined") {
            throw new Error("WalletConnect is not initialized");
        }
        // populates existing pairings to state
        this.pairings = _client.pairing.getAll({ active: true });
        this.logger && console.log("RESTORED PAIRINGS: ", _client.pairing.getAll({ active: true }));
        ...
        if (_client.session.length) {
            ...
            window.wcClient = _client;
            window.wcSession = _session;
            this.session = _session;
            return _session;
        }
    }
    ...
    async _connect(pairing) {
        ...
        try {
            ...
            this.logger && console.log("requiredNamespaces config for connect:", requiredNamespaces);
...
//# sourceMappingURL=Web3ModalConnector.js.map